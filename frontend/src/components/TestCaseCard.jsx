...
import { TestTube2, AlertCircle, CheckCircle, Zap } from 'lucide-react';
        ...
        {testCases.map((tc, idx) => (
          <div key={idx} className="bg-slate-950/70 border border-slate-800/80 rounded-lg p-3 flex flex-col gap-2">
            <div className="flex items-center justify-between">
              ...
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded border ${getBadgeStyle(tc.name)}`}>
                {tc.name.split(' ')[0] || 'Test'}
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-mono">
              ...
              <div className="bg-slate-900/90 border border-slate-800 rounded p-2">
                ...
                <span className="text-emerald-400 break-all">{tc.expectedOutput}</span>
              </div>
            </div>

            {tc.reason && (
              <p className="text-[11px] text-slate-400 italic bg-slate-900/40 px-2.5 py-1 rounded border border-slate-800/40">
                💡 <span className="font-medium text-slate-300">Why it matters:</span> {tc.reason}
              </p>
            )}
          </div>
        ))}
